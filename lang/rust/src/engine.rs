...
impl<S: FlowState> FlowEngine<S> {
    ...
    pub fn set_transition_logger(
        &mut self,
        logger: impl Fn(&TransitionLogEntry) + Send + Sync + 'static,
    ) {
    ...
    pub fn take_transition_logger(
        &mut self,
    ) -> Option<Box<dyn Fn(&TransitionLogEntry) + Send + Sync>> {
        self.transition_logger.take()
    }
    pub fn take_state_logger(&mut self) -> Option<Box<dyn Fn(&StateLogEntry) + Send + Sync>> {
        self.state_logger.take()
    }
    pub fn take_error_logger(&mut self) -> Option<Box<dyn Fn(&ErrorLogEntry) + Send + Sync>> {
        self.error_logger.take()
    }
    pub fn take_guard_logger(&mut self) -> Option<Box<dyn Fn(&GuardLogEntry) + Send + Sync>> {
        self.guard_logger.take()
    }
    ...
    pub fn start_flow(
        ...
        for (type_id, value) in initial_data {
            ctx.put_raw(type_id, value);
        }
    ...
    pub fn resume_and_execute(
        ...
        let transition_info = {
            let flow = self.store.get_mut(flow_id).ok_or_else(|| {
                FlowError::new(
                    "FLOW_NOT_FOUND",
                    format!("Flow {flow_id} not found or completed"),
                )
            })?;

            for (tid, val) in external_data {
                flow.context.put_raw(tid, val);
            }
                            ...
                            if let Some(proc) = &transition.processor {
                                match proc.process(&mut flow.context) {
                                    ...
                                    Ok(()) => {
                                        ...
                                        if let Some(action) = def.exit_action(current) {
                                            action(&mut flow.context);
                                        }
                                        flow.transition_to(to);
                                        if let Some(action) = def.enter_action(to) {
                                            action(&mut flow.context);
                                        }
                                        Some((
                                            from_dbg,
    ...
    fn execute_auto_chain(&mut self, flow_id: &str) -> Result<(), FlowError> {
        ...
        while depth < max_depth {
            ...
            let step_result = {
                let flow = match self.store.get_mut(flow_id) {
                    Some(f) => f,
                    None => break,
                };
                let current = flow.current_state();
                if current.is_terminal() {
                    flow.complete(format!("{:?}", current));
                    break;
                }
            ...
            let Some((from, to, trigger, is_error)) = step_result else {
                break;
            };
            ...
            SubFlowResult::Completed(exit_name) => {
                ...
                if let Some(config) = config {
                    if let Some(&target) = config.exit_mappings.get(&exit_name) {
                        ...
                        if let Some(action) = def.enter_action(target) {
                            action(&mut flow.context);
                        }
                        return Ok(Some((
                            from,
    ...
    fn dispatch_step(
            ...
            .find(|t| t.from == current && t.transition_type == TransitionType::Auto)
        {
            if let Some(proc) = &t.processor {
                ...
                let strict_fail = result.is_ok()
                    && (strict_mode || def.strict_mode)
                ...
                if let Some(action) = def.enter_action(t.to) {
                    action(&mut flow.context);
                }
                return Ok(Some((
                    from_dbg,
            ...
            if let Some(branch) = &t.branch {
                ...
                if let Some(&target) = t.branch_targets.get(&label) {
                    ...
                    let specific = def
                        ...
                        .find(|tr| {
                            tr.from == current
                                && tr.transition_type == TransitionType::Branch
                                && tr.branch_label.as_deref() == Some(&label)
                        })
                        .or_else(|| {
                            def.transitions.iter().find(|tr| {
                                tr.from == current
                                    && tr.transition_type == TransitionType::Branch
                                    && tr.to == target
                            })
    ...
    fn handle_error_with_cause(
            ...
            if let Some(routes) = def.exception_routes.get(&from_state) {
                for route in routes {
                    if (route.predicate)(cause) {
                        ...
                        if route.target.is_terminal() {
                            flow.complete(format!("{:?}", route.target));
                        }
                        return;
                    }
                }
        ...
        if let Some(&err_target) = def.error_transitions.get(&from_state) {
            ...
            if err_target.is_terminal() {
                flow.complete(format!("{:?}", err_target));
            }
        } else {
