...
export class DataFlowGraph {
    ...
    explain(state, key) {
        ...
        const result = { state, available: new Set(available), missing: [] };
        ...
        if (key) {
            ...
            keysToCheck = [];
            for (const [typeName, nodes] of this._consumers) {
                if (nodes.some(n => n.fromState === state))
                    keysToCheck.push(typeName);
            }
        }
     ...
     */
    whyMissing(key, state) {
        const k = key;
        const available = this.availableAt(state);
        if (available.has(k))
            return [`'${k}' IS available at ${state}`];
        const lines = [];
        const producers = this._producers.get(k) ?? [];
