...
階層が必要なら [SubFlow](docs/example-oidc-auth-flow-ja.md)（ネストではなく合成）を使う。並行する関心事は[別フロー](docs/patterns/long-lived-flows.md)にして `crossFlowMap()` でリンク。
...
const stateConfig: Record<OrderState, StateConfig> = {
    ...
    SHIPPED:           { terminal: true,  initial: false },  // 終端 — フローはここで終わる
    CANCELLED:         { terminal: true,  initial: false },  // 終端 — エラー終了
};
```
